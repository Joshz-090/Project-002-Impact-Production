...
import QuickQuestionButton from "../QuickQuestionButton/QuickQuestionButton";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex absolute left-1/2 transform -translate-x-1/2">
            <div
              className={`flex space-x-6 px-4 py-1 rounded-full transition-colors duration-500 ${
                isScrolled ? "bg-black bg-opacity-80" : "bg-black bg-opacity-40"
              }`}
            >
              {navLinks.map((link) => (
                <div key={link.path} className="relative group">
                  ...
                  {/* Improved Tooltip */}
                  <div className="absolute top-full left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none mt-2">
                    <div className="bg-black bg-opacity-80 text-white text-xs font-sans font-bold uppercase tracking-wider px-2 py-1 rounded whitespace-nowrap">
                      {link.name}
                    </div>
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 flex flex-col">
            {navLinks.map((link) => (
              <Link
                ...
                to={link.path}
                onClick={closeMenu}
                className={`font-sans font-bold uppercase tracking-wider px-3 py-2 rounded-md text-base flex items-center ${
                  location.pathname === link.path
                    ? "text-yellow-500 bg-black"
                    : "text-white hover:text-yellow-400 hover:bg-opacity-75"
                }`}
              >
                <svg
                  width={20}
            ...
            <Link
              to="/contact"
              onClick={closeMenu}
              className="bg-yellow-500 text-black font-sans font-bold uppercase tracking-wider px-3 py-2 rounded text-center mt-2 hover:bg-opacity-90 transition-colors duration-300 flex items-center justify-center"
            >
              <svg
                width={16}
                height={16}
